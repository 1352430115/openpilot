...
class CarState(CarStateBase, CarStateExt):
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    ret.brakeLightsDEPRECATED = bool(cp.vl["PCM_CRUISE"]["PERMIT_BRAKING"])
    if self.CP.flags & ToyotaFlags.SECOC.value:
      ...
      if not self.CP.flags & ToyotaFlags.DISABLE_RADAR.value:
        ret.stockAeb = bool(cp_acc.vl["ACC_CONTROL"]["PRECOLLISION_ACTIVE"] and cp_acc.vl["PRE_COLLISION"]["FORCE"] < -1e-5)
